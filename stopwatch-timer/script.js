...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyFsTimerCount() {
    if (getFsTimerCount() === '1') {
      document.body.classList.add('fs-one-timer');
      document.body.classList.remove('fs-two-timers');
    } else {
      document.body.classList.add('fs-two-timers');
      document.body.classList.remove('fs-one-timer');
    }
  }

  fsTimerCountRadios.forEach(r => {
  ...
  function enterFullscreen(mode) {
    ...
    if (mode === 'timer') {
      ...
      document.body.classList.remove('fs-one-timer', 'fs-two-timers');
    }
  ...
  if (swFullscreenBtn) {
    swFullscreenBtn.addEventListener('click', () => toggleFullscreen('stopwatch'));
  }
  if (tmFullscreenBtn) {
  ...
  document.addEventListener('webkitfullscreenchange', () => {
    if (!document.webkitFullscreenElement && document.body.classList.contains('fs-active')) {
      exitFullscreen();
    }
  });

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && document.body.classList.contains('fs-active')) {
      exitFullscreen();
    }
  });
});
