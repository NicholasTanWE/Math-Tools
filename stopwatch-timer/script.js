...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let lastChimeAt = 0; // performance.now() of last chime (for simultaneous dedupe)
  ...
  /**
   * @param {number} count  1 = single, 2 = double
   */
  function playChime(count) {
    ...
    try {
      ...
      if (count >= 2) {
        playTone(ctx, now + 0.45, 0.35, 880);
      }
    } catch (e) { /* ignore */ }
  }
   ...
   * @param {'1'|'2'} timerId
   * @param {object} otherTimer  the other timer controller (has isRunning, getRemainingMs)
   */
  function handleFinishChime(timerId, otherTimer) {
    if (soundMuted) return;

    const now = performance.now();
    // Other timer also finishing within 200 ms → treat as simultaneous → one single chime only
    const otherAlmostDone = otherTimer && (
      (typeof otherTimer.getRemainingMs === 'function' && otherTimer.getRemainingMs() <= 200) ||
      otherTimer.isFinished()
    );

    if (otherAlmostDone || (now - lastChimeAt < 200)) {
      // Simultaneous finish: play only one single chime, and only if we haven't already
      if (now - lastChimeAt >= 200) {
        lastChimeAt = now;
        playChime(1);
      }
      return;
    ...
    lastChimeAt = now;
    // Distinct chimes: Timer 1 = single, Timer 2 = double
    playChime(timerId === '2' ? 2 : 1);
  }

  // Populated after both createTimer calls so finish-chime can see the other timer
  const timers = {};

  /**
   * Creates an independent timer controller for one panel.
   * id is "1" or "2"
   */
  function createTimer(id) {
    ...
    function setDisplays(ms) {
      ...
      // Progress: fill left→right as time elapses (0% at start → 100% at finish)
      if (totalDuration > 0) {
    ...
    function tick() {
      ...
      if (left <= 0) {
        ...
        handleFinishChime(id, timers[otherId]);
        updateSharedButtons();
        return;
      }
      setDisplays(left);
    ...
    function onDurationChange() {
      ...
      const ms = getDurationMs();
      totalDuration = 0; // not started yet
      setDisplays(ms);
    }
    ...
    return {
      ...
      getRemainingMs: () => running ? Math.max(0, endTime - performance.now()) : remaining
    };
  }
  ...
  function updateSharedButtons() {
    ...
    // Start Both enabled if at least one can still be started
  ...
  tmStartBothBtn.addEventListener('click', () => {
    // Start whichever are not already running
